...
{
    public class ProxyStudent : Person, IStudent
    {
        protected const int studentType = 1;
        protected const int StudentIntelligentType = 2;
        ...
        public ProxyStudent(string name, int dni, int id, double average, double examScore) : base(name, dni)
        {
            this.studentID = id;
            this.average = average;
            this.examScore = examScore;
            comparableStrategy = new CompareByStudentId();
            pe = DecoratorFactory.CreatePrintQualification((IStudent)this);
            student = null;
        }
        ...
        public virtual int AnswerQuestion(int question)
        {
            if (student == null)
            {
                student = StudentFactory.StudentFabricator(studentType, name, dni, studentID, average, examScore);
            }
            Console.WriteLine($"Creando alumno desde proxy");
